...
{
    "Keys": [
        { "AttributeName": "yokai_id", "KeyType": "HASH" }
    ],
    "Attrs": [
        { "AttributeName": "yokai_id", "AttributeType": "S" }
    ],
    "GSIs": null,
    "ItemCount": 174,
    "Size": 215911,
...
**所見:**
- PK: `yokai_id` (String) のみ。SKなし、GSIなし。
- 174件、約211KB。
- location 検索（lat/lon）は GSI なし → 現状フルスキャンで実現していると推定。

### 2. `kodama_db` scan サンプル（属性一覧）
...
| `name` | S | core |
| `kana` | S | core |
| `appearance` | S | core（本文） |
| `notes` | S | core（解説） |
| `regions` | L(S) | core |
| `region_detail` | S | core |
| `category_tags` | L(S) | core |
| `keywords` | L(S) | core |
| `images` | L(S) | core（画像パス） |
| `image_types` | L(S) | core |
| `image_captions` | L(S) | core |
| `latitude` | N | core（配置座標） |
| `longitude` | N | core（配置座標） |

**所見:**
- すべての属性が「配信用本体（core）」の責務に相当。
- `research`（査読用メタ: 情報源URL、信頼性スコア、収集日時、収集者、ステータス）は現テーブルには存在しない。
- 文字コードが Shift-JIS 系でAPIレスポンスが文字化けする可能性あり（要確認）。
- `images` は `images/{slug}.png` 形式（S3 URI でなく相対パス）。
...
```json
{
    "Keys": [
        ...
        { "AttributeName": "youkaiId", "KeyType": "RANGE" }
    ],
    "Attrs": [
        { "AttributeName": "deviceId", "AttributeType": "S" },
        { "AttributeName": "youkaiId", "AttributeType": "S" }
    ],
    "GSIs": null,
    "ItemCount": 42
}
...
**所見:**
- PK: `deviceId` / SK: `youkaiId`。GSIなし。
- 特定デバイスの捕獲一覧は効率的にクエリ可能。逆引き（特定妖怪を捕獲したデバイス一覧）はスキャン必要。

### 4. `youkai-collection-images` s3 ls（抜粋）

```
youkai/                                        (prefix)
youkai/{uuid}_camera.png                       (新形式: UUIDをキーに)
youkai/{yokai_slug}_camera.png                 (旧形式: 名前スラッグをキーに)
```

実際のキー例:
- `youkai/4248ed6e-77ec-42f5-9ff7-59c2e96f365b_camera.png` (UUID形式・新)
- `youkai/aburasumashi_camera.png` (スラッグ形式・旧)

**所見:**
- キー構造は `youkai/{id}_camera.png` フラット構造。サブディレクトリなし。
- 旧データ（スラッグ形式）と新データ（UUID形式）が混在。
- `media_attachments`（research 用添付）は別 prefix（例: `research/{id}/attachments/`）で切り分けが妥当。
